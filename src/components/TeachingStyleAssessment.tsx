...
export function TeachingStyleAssessment({ onComplete, onBack, initialResponses, initialQuestions, onSaveProgress }: TeachingStyleAssessmentProps) {
  ...
  const [responses, setResponses] = useState<number[]>(
    initialResponses && initialResponses.length > 0 ? initialResponses : new Array(teachingStyleQuestions.length).fill(0)
  );
  ...
  const generateAssessmentSubset = () => {
    ...
    const pools = {
        authorityPos: [7, 18, 20, 22, 24],
        authorityNeg: [1, 2, 6, 17, 19, 21, 23],
        knowledgePos: [11, 12, 13, 15, 16],
        knowledgeNeg: [9, 10, 14],
        assessmentPos: [34, 35, 36, 38, 40],
        assessmentNeg: [33, 37, 39],
        climatePos: [50, 51, 53, 54, 55],
        climateNeg: [49, 52, 56],
        adaptPos: [42, 43, 44, 45, 47, 48],
        adaptNeg: [41, 46],
        motivationAll: [25, 26, 27, 28, 29, 30, 31, 32],
        innovationAll: [57, 58, 59, 60, 61, 62, 63, 64]
    };
        ...
        <div className="flex items-center justify-between sticky top-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md p-4 rounded-xl shadow-sm border">
          ...
          <div className="flex items-center gap-2">
             <Badge variant="secondary" className="text-sm hidden sm:inline-flex">
                ...
                {isFullAssessment ? "Full Diagnostic" : "Adaptive Assessment"}
            </Badge>
          ...
          <CardContent className="pt-6">
            <div className="space-y-2">
              ...
              <p className="text-xs text-center text-muted-foreground mt-2">
                  {isFullAssessment 
                    ? `Answering all ${activeQuestions.length} questions for a comprehensive analysis.` 
